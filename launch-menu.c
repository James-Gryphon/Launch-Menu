...
static void enforce_item_limit_with_timestamps(const gchar *folder_path, guint max_items)
{
...
static gchar *calculate_reduced_path(const gchar *target_path, GList *all_items) 
{
...
static void create_clean_links(const gchar *folder_path, LaunchMenuPlugin *launch_plugin) 
{
    #ifdef DEBUG
    ...
    if (dir) 
    {
        ...
        while ((entry = readdir(dir)) != NULL) 
        {
            ...
            gchar *old_link = g_build_filename(folder_path, entry->d_name, NULL);
            unlink(old_link);
            g_free(old_link);
        }
        closedir(dir);
    ...
    if (dir) 
    {
        ...
        while ((entry = readdir(dir)) != NULL) 
        {
            ...
            {
                gchar *folder_to_remove = g_build_filename(folder_path, entry->d_name, NULL);
                /* Recursively remove folder and contents */
                if (g_file_test(folder_to_remove, G_FILE_TEST_IS_DIR)) 
                {
                    gchar *rm_command = g_strdup_printf("rm -rf '%s'", folder_to_remove);
    ...
    GHashTable *name_groups = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, (GDestroyNotify)g_list_free);
    GHashTableIter iter;
    gpointer key, value;
...
static void parse_xbel_file(LaunchMenuPlugin *launch_plugin) 
{
    ...
    gchar *recent_docs_path = g_build_filename(launch_plugin->launch_menu_path, RECENT_DOCS_DIR, NULL);
    gchar *timestamps_dir = g_build_filename(recent_docs_path, ".timestamps", NULL);
    g_mkdir_with_parents(timestamps_dir, 0755);
    /* Track seen filenames for Mac-style deduplication */
    GHashTable *seen_names = NULL;
    if (launch_plugin->classic_style_duplicates) 
    {
        seen_names = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
    }
    ...
    for (xmlNode *current = root->children; current; current = current->next) 
    {
        ...
        #endif
        /* Check if this bookmark is from a blacklisted application */
        ...
        if (href && (modified || added)) 
        {
            ...
            if (g_str_has_prefix(file_uri, "file://")) 
            {
                ...
                if (local_path && g_file_test(local_path, G_FILE_TEST_EXISTS) && is_document_file(local_path)) 
                {
    ...
    gint count = 0;
    for (GList *l = xbel_entries; l && count < launch_plugin->recent_docs_count; l = l->next, count++)
    {
        ...
        for (GList *e = existing_timestamps; e; e = e->next) 
        {
            ...
            if (g_strcmp0(new_entry->path, existing->path) == 0) 
            {
                ...
                existing_timestamps = g_list_remove(existing_timestamps, existing);
    ...
    #ifdef DEBUG
    g_debug("XBEL_DEBUG: Deleting %d obsolete files:\n", g_list_length(existing_timestamps));
    #endif
    for (GList *l = existing_timestamps; l; l = l->next) 
    {
        RecentItem *item = l->data;
        gchar *old_link = g_build_filename(timestamps_dir, item->timestamp_filename, NULL);
        #ifdef DEBUG
        g_debug("  Deleting: %s -> ", old_link);
        #endif
    ...
    launch_plugin->recent_docs_count; l = l->next, count++) 
    {
        ...
        {
            #ifdef DEBUG
            g_debug("XBEL_ERROR: NULL entry at position %d\n", count);
            #endif
            continue;
        }
        XbelEntry *entry = (XbelEntry *)l->data;
        #ifdef DEBUG
        g_print("XBEL_CREATE: [%d] '%s' timestamp=%ld\n", count, entry->name ? entry->name : "NULL", entry->timestamp);
        #endif
        gchar *link_name = create_safe_filename(entry->name, entry->timestamp);
        gchar *link_path = g_build_filename(timestamps_dir, link_name, NULL);

        if (symlink(entry->path, link_path) != 0) 
        {
            g_warning("Failed to create symlink from %s to %s: %s", link_path, entry->path, strerror(errno));
        }
        #ifdef DEBUG
...
static void on_xbel_file_changed(GFileMonitor *monitor G_GNUC_UNUSED, GFile *file G_GNUC_UNUSED, GFile *other_file G_GNUC_UNUSED, GFileMonitorEvent event_type, gpointer user_data) 
{
...
static void setup_application_monitoring(LaunchMenuPlugin *launch_plugin) 
{
    /* Initialize hash table for tracking known applications */
    ...
    /* Force initial update and populate known applications */
    wnck_screen_force_update(launch_plugin->wnck_screen);
    populate_initial_applications(launch_plugin);
}

/* Populate initial applications list to avoid spurious "new" detections */
static void populate_initial_applications(LaunchMenuPlugin *launch_plugin) 
{
    ...
    window_list; window_list = window_list->next) 
    {
        ...
        this application */
        if (g_hash_table_lookup(seen_apps, app)) 
        {
            continue;
        }
...
static void on_application_opened(WnckScreen *screen, WnckApplication *app, gpointer user_data) 
{
    (void)screen;
    LaunchMenuPlugin *launch_plugin = (LaunchMenuPlugin *)user_data;
    if (!launch_plugin->recents_on) return;
    const gchar *app_name = wnck_application_get_name(app);
    if (!app_name || strlen(app_name) == 0 || classlib_is_file_manager(app)) 
    {
        return;
    }
...
static void add_recent_application_from_wnck(LaunchMenuPlugin *launch_plugin, WnckApplication *app)
{
    ...
    if (desktop_path) 
    {
        ...
        {
            create_recent_application_entry_with_desktop(launch_plugin, display_name ? display_name : app_name, desktop_path);
            g_free(display_name);
            g_free(icon_name);
        }
        g_free(desktop_path);
...
/* Create recent application entry using desktop file */
static void create_recent_application_entry_with_desktop(LaunchMenuPlugin *launch_plugin,
const gchar *display_name, const gchar *desktop_path) 
{
    gchar *recent_apps_path = g_build_filename(launch_plugin->launch_menu_path, RECENT_APPS_DIR, NULL);
    gchar *timestamps_dir = g_build_filename(recent_apps_path, ".timestamps", NULL);
    g_mkdir_with_parents(timestamps_dir, 0755);
    /* Remove any existing timestamp entries for this app */
    ...
    if (ts_dir) 
    {
        ...
        {
            if (strstr(entry->d_name, display_name)) 
            {
    ...
    /* Create clean symlink with .desktop extension */
    ...
    if (symlink(desktop_path, existing_link) != 0) 
    {
        g_warning("Failed to create symlink: %s -> %s", existing_link, desktop_path);
    }

    /* Enforce limits */
    enforce_item_limit_with_timestamps(recent_apps_path, launch_plugin->recent_apps_count);
...
static void create_simple_recent_application_entry(LaunchMenuPlugin *launch_plugin, const gchar *app_name, WnckApplication *app) 
{
    ...
    if (g_file_test(desktop_path, G_FILE_TEST_EXISTS)) 
    {
        ...
        create_recent_application_entry_with_desktop(launch_plugin, app_name, desktop_path);
        goto cleanup; // (I always wanted to use one of these somewhere)
    }
    ...
    // Now create recent entry with this desktop file /
...
static void cleanup_application_monitoring(LaunchMenuPlugin *launch_plugin)
{
    if (launch_plugin->wnck_screen) 
    {
        g_signal_handlers_disconnect_by_data(launch_plugin->wnck_screen, launch_plugin);
    }
...
/* Get appropriate display name for a menu item */
static gchar *get_item_display_name(const gchar *item_name, const gchar *item_path) 
{
    gchar *display_name = NULL;
    /* Check if it's a desktop file or symlink to a desktop file */
    if (g_str_has_suffix(item_name, ".desktop")) {
    gchar *temp_display = NULL;
    gchar *temp_icon = NULL;
    if(parse_desktop_file_for_menu(item_path, &temp_display, &temp_icon)) 
    {
        display_name = temp_display; 
        /* Transfer ownership */
        g_free(temp_icon); 
        /* We don't need icon here */
    }
    } 
    else if (g_file_test(item_path, G_FILE_TEST_IS_SYMLINK)) 
    {
        gchar *target = g_file_read_link(item_path, NULL);
        if (target && g_str_has_suffix(target, ".desktop")) 
        {
            gchar *temp_display = NULL;
            gchar *temp_icon = NULL;
            if (parse_desktop_file_for_menu(target, &temp_display, &temp_icon)) 
            {
                display_name = temp_display;
                /* Transfer ownership */
                g_free(temp_icon); 
    ...
    /* Fall back to original filename if no display name found */
    if (!display_name) 
    {
...
static gchar *launch_menu_find_icon(void) 
{
    ...
    {
        if(gtk_icon_theme_has_icon(icon_theme, icon_candidates[i])) 
        {
            return g_strdup(icon_candidates[i]);
        }
    } // implicit else
    return g_strdup("xfce4_xicon1");
}

/* Enhanced create menu item with icon - desktop file support */
static GtkWidget *create_menu_item_with_icon(const gchar *label, const gchar *item_path) 
{
    ...
    if (item_path && g_file_test(item_path, G_FILE_TEST_IS_SYMLINK)) 
    {
        gchar *target = g_file_read_link(item_path, NULL);
        if (target && g_str_has_suffix(target, ".desktop")) 
        {
            /* It's a symlink to a desktop file - parse it for pretty name and icon */
            parse_desktop_file_for_menu(target, &display_name, &icon_name);
        }
        g_free(target);
    }
    else if (item_path && g_str_has_suffix(item_path, ".desktop")) 
    {
        /* It's a direct desktop file - parse it */
        parse_desktop_file_for_menu(item_path, &display_name, &icon_name);
    }
    /* Use parsed display name if available, otherwise use provided label */
    const gchar *final_label = display_name ? display_name : label;
    ...
    else if (item_path) 
    {
        ...
        {
            /* Check for custom folder icon via GIO metadata */
            ...
            if (custom_icon && strlen(custom_icon) > 0) 
            {
        ...
        else if (g_file_test(item_path, G_FILE_TEST_IS_EXECUTABLE) || g_str_has_suffix(item_path, ".desktop"))
        {
            /* It's executable or desktop file - try to get application icon */
            GFile *file = g_file_new_for_path(item_path);
            GFileInfo *file_info = g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_ICON, G_FILE_QUERY_INFO_NONE, NULL, NULL);
            if (file_info) 
            {
        ...
        {
            /* It's a regular file - try to get file type icon */
            GFile *file = g_file_new_for_path(item_path);
    ...
    /* If we still don't have an icon, use a generic one */
    if (!icon) 
    {
...
/* Comparison function for recent items - old style for compatibility */
typedef struct {
gchar *name;
time_t mtime;
} RecentItemOld;
...
/*
static gint compare_recent_items_old(gconstpointer a, gconstpointer b) 
{
    const RecentItemOld *item_a = (const RecentItemOld *)a; 
    const RecentItemOld *item_b = (const RecentItemOld *)b;
    if (item_a->mtime > item_b->mtime) return -1;
    if (item_a->mtime < item_b->mtime) return 1;
    return 0;
}
*/
/* Cleanup recent folder - enforce strict item limit (legacy function for compatibility)
static void cleanup_recent_folder(const gchar *folder_path, gint max_items) 
{
    if (!g_file_test(folder_path, G_FILE_TEST_IS_DIR)) 
    {
    ...
    // Collect all items with their modification times
    while ((entry = readdir(dir)) != NULL) 
    {
        if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0) 
        {
    ...
    items = g_list_sort(items, compare_recent_items_old);
...
/* Add recent application - basic implementation */
/*
static void add_recent_application(LaunchMenuPlugin *launch_plugin, const gchar *app_path)
{
    if (!launch_plugin || !app_path || !g_file_test(app_path, G_FILE_TEST_EXISTS)) 
    {
        return;
    }
    ...
    if (symlink(app_path, link_path) == 0) 
    {
        ...
        #endif
        // Cleanup old items - enforce strict limit
...
/* Enhanced launch item click handler with desktop file support */
static void on_launch_item_clicked(GtkMenuItem *item, gpointer user_data) 
{
    gchar *item_path = (gchar *)user_data;

    if (!item_path || !g_file_test(item_path, G_FILE_TEST_EXISTS)) 
    {
        return;
    }
    ...
    targets */
    if (g_file_test(item_path, G_FILE_TEST_IS_SYMLINK)) 
    {
        gchar *target = g_file_read_link(item_path, NULL);
        if (target && g_file_test(target, G_FILE_TEST_EXISTS)) 
        {
            item_path = target;  
            /* Use resolved target for all operations */
        }
        else 
        {
            g_free(target);
    ...
    if (g_str_has_suffix(item_path, ".desktop")) 
    {
    ...
    else if (g_file_test(item_path, G_FILE_TEST_IS_DIR)) 
    {
    ...
    else if (g_file_test(item_path, G_FILE_TEST_IS_EXECUTABLE)) 
    {
        /* It's a regular executable - run it and track as recent application */
        g_spawn_command_line_async(item_path, &error);
        /* Track as recent application
        LaunchMenuPlugin *launch_plugin = g_object_get_data(G_OBJECT(item), "launch-menu-plugin");
        if (launch_plugin) 
        {
    ...
    {
        /* It's a regular file - open with default application */
        gchar *uri = g_filename_to_uri(item_path, NULL, NULL);
        if (uri) 
        {
            gtk_show_uri_on_window(NULL, uri,
...
/* Build submenu from directory - enhanced with natural sorting */
static GtkWidget *build_submenu_from_directory(const gchar *dir_path, gint depth, LaunchMenuPlugin *launch_plugin) 
{
    /* Limit recursion depth to prevent overly deep menus */
    if (depth >= 3) 
    {
        return NULL;
    }
    ...
    g_signal_connect(open_folder_item, "activate", G_CALLBACK(on_launch_item_clicked), g_strdup(dir_path));
    ...
    if (!dir) 
    {
        return submenu; /* Return menu with just "Open This Folder" */
    }

    struct dirent *entry;
    GList *menu_items = NULL;

    /* Collect items with display names for sorting */
    while ((entry = readdir(dir)) != NULL) 
    {
        if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0) 
        {
    ...
    for (GList *l = menu_items; l; l = l->next) 
    {
        ...
        if (g_str_has_suffix(item_info->display_name, "|")) 
        {
            // Last character matches | - this is similar to DividerLines, which used $
            GtkWidget *separator = gtk_separator_menu_item_new();
            gtk_menu_shell_append(GTK_MENU_SHELL(submenu), separator);
            continue;
        }
        ...
        if (g_file_test(item_info->full_path, G_FILE_TEST_IS_DIR)) 
        {
            /* It's a directory - create submenu */
            GtkWidget *nested_submenu = build_submenu_from_directory(item_info->full_path, depth + 1, launch_plugin);
            if (nested_submenu) 
            {
                /* Check if the submenu has content beyond "Open This Folder" */
                ...
                if (submenu_children && child_count > 2) 
                {
                    /* Submenu has items beyond "Open This Folder" + separator */
                    gtk_menu_item_set_submenu(GTK_MENU_ITEM(menu_item), nested_submenu);
                } 
                else 
                {
                    /* Submenu only has "Open This Folder" - destroy it and connect folder click instead */
            ...
            {
                /* No submenu created - connect folder click */
                g_signal_connect(menu_item, "activate", G_CALLBACK(on_launch_item_clicked), g_strdup(item_info->full_path));
            }
        } 
        else 
        {
            /* It's a file - connect the click handler and set plugin data for recent tracking */
...
static gboolean launch_menu_check_hardinfo(void) 
{
    gchar *hardinfo_path = g_find_program_in_path("hardinfo2");
    if (hardinfo_path) 
    {
...
/* Create directories */
static void create_launch_directories(LaunchMenuPlugin *launch_plugin) 
{
    if (!launch_plugin || !launch_plugin->launch_menu_path) 
    {
        return;
    ...
    gchar *settings_symlink = g_build_filename(launch_plugin->launch_menu_path, "Settings Manager.desktop", NULL);
    if (!g_file_test(settings_symlink, G_FILE_TEST_EXISTS)) 
    {
        symlink("/usr/share/applications/xfce-settings-manager.desktop", settings_symlink);
    }
...
static void create_simple_menu(LaunchMenuPlugin *launch_plugin) 
{
    ...
    if (launch_plugin->hardinfo_available) 
    {
        ...
        g_signal_connect(about_item, "activate", G_CALLBACK(on_about_computer_clicked), NULL);
    ...
    /* Add launch menu items from directory with enhanced sorting */
    if(g_file_test(launch_plugin->launch_menu_path, G_FILE_TEST_IS_DIR)) 
    {
        ...
        if (dir) 
        {
            ...
            /* Collect items with display names */
            while ((entry = readdir(dir)) != NULL) 
            {
                if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0) 
                {
                ...
                MenuItemInfo *item_info = g_new0(MenuItemInfo, 1);
                item_info->filename = g_strdup(entry->d_name);
                if (!launch_plugin->recents_on && (g_strcmp0(item_info->filename, RECENT_APPS_DIR) == 0 || g_strcmp0(item_info->filename, RECENT_DOCS_DIR) == 0)) 
                {
                    continue;
                }
                item_info->display_name = display_name;
            ...
            /* Create menu items with submenu support using sorted list */
            for (GList *l = menu_items; l; l = l->next) 
            {
                MenuItemInfo *item_info = (MenuItemInfo *)l->data;
                if (g_str_has_suffix(item_info->display_name, "|")) 
                {
                    // Last character matches | - this emulates DividerLines
                    GtkWidget *separator = gtk_separator_menu_item_new();
                    gtk_menu_shell_append(GTK_MENU_SHELL(launch_plugin->menu), separator);
                    continue;
                }
                ...
                {
                    /* It's a file - connect click handler and set plugin data for recent tracking */
                    g_object_set_data(G_OBJECT(menu_item), "launch-menu-plugin", launch_plugin);
                    g_signal_connect(menu_item, "activate", G_CALLBACK(on_launch_item_clicked),
                    g_strdup(item_info->full_path));
                }
            ...
            g_list_free_full(menu_items, (GDestroyNotify)menu_item_info_free);
        }
    } 
    else 
    {
        ...
        gtk_widget_set_sensitive(no_items, FALSE);
...
static gboolean on_button_pressed(GtkWidget *widget, GdkEventButton *event, LaunchMenuPlugin *launch_plugin) 
{
    if (event->button == 1) 
    { /* Left mouse button */
        ...
        if (launch_plugin->menu) 
        {
            gtk_menu_popup_at_widget(GTK_MENU(launch_plugin->menu), widget, GDK_GRAVITY_SOUTH_WEST, GDK_GRAVITY_NORTH_WEST, (GdkEvent*)event);
        }
...
static gchar *launch_menu_get_property_name(LaunchMenuPlugin *plugin, const gchar *property)
{
...
static void launch_menu_configure_plugin(XfcePanelPlugin *panel, LaunchMenuPlugin *plugin)
{
    ...
    /* Create dialog */
    dialog = xfce_titled_dialog_new_with_mixed_buttons(_("Launch Menu Properties"),

    GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(panel))), GTK_DIALOG_DESTROY_WITH_PARENT, "window-close-symbolic", _("_Close"), GTK_RESPONSE_CLOSE, NULL);
    gtk_window_set_icon_name(GTK_WINDOW(dialog), "xfce4_xicon1");
    ...
    /* Recent apps field
    * Create spin button for max recent items (10-99)
    */
    recent_apps_count_spin = gtk_spin_button_new_with_range(10.0, 99.0, 1.0);

    gtk_spin_button_set_value(GTK_SPIN_BUTTON(recent_apps_count_spin), plugin->recent_apps_count);
    /* Add label */
    ...
    gtk_box_pack_start(GTK_BOX(recent_apps_hbox), recent_apps_label, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(recent_apps_hbox), recent_apps_count_spin, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(vbox), recent_apps_hbox, FALSE, FALSE, 0);
    ...
    */
    ...
    gtk_spin_button_set_value(GTK_SPIN_BUTTON(recent_docs_count_spin), plugin->recent_docs_count);
    /* Add label */
    ...
    gtk_box_pack_start(GTK_BOX(recent_docs_hbox), recent_docs_label, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(recent_docs_hbox), recent_docs_count_spin, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(vbox), recent_docs_hbox, FALSE, FALSE, 0);

    /* Connect signals */
    g_signal_connect(recents_on_check, "toggled", G_CALLBACK(on_recents_toggled), plugin);
    g_signal_connect(classic_dups_check, "toggled", G_CALLBACK(on_classic_dups_toggled), plugin);
    g_signal_connect(recent_apps_count_spin, "value-changed", G_CALLBACK(on_recent_apps_count_spun), plugin);
    g_signal_connect(recent_docs_count_spin, "value-changed", G_CALLBACK(on_recent_docs_count_spun), plugin);
    /* Sensitivity controls*/
    g_signal_connect(recents_on_check, "toggled", G_CALLBACK(sensitivity_changer), classic_dups_check);
    g_signal_connect(recents_on_check, "toggled", G_CALLBACK(sensitivity_changer), recent_apps_count_spin);
    g_signal_connect(recents_on_check, "toggled", G_CALLBACK(sensitivity_changer), recent_docs_count_spin);
...
static void launch_menu_construct(XfcePanelPlugin *plugin) 
{
    ...
    launch_plugin->property_base = g_strdup_printf("%s/plugin-%d", CONFIG_PROPERTY_BASE, xfce_panel_plugin_get_unique_id(launch_plugin->plugin));
    ...
    /* Make button focusable for automation tools */
    ...
    gtk_box_pack_start(GTK_BOX(hbox), launch_plugin->icon, FALSE, FALSE, 0);
    launch_plugin->label = gtk_label_new("Launch");
    gtk_box_pack_start(GTK_BOX(hbox), launch_plugin->label, TRUE, TRUE, 0);
    gtk_container_add(GTK_CONTAINER(launch_plugin->button), hbox);

    /* Connect signal */
    g_signal_connect(launch_plugin->button, "button-press-event",
    ...
    g_signal_connect(plugin, "configure-plugin", G_CALLBACK(launch_menu_configure_plugin), launch_plugin);
    g_signal_connect(plugin, "about", G_CALLBACK(launch_menu_about), NULL);

    /* Add to panel */
    gtk_container_add(GTK_CONTAINER(plugin), launch_plugin->button);
    xfce_panel_plugin_add_action_widget(plugin, launch_plugin->button);

    /* Store plugin data */
    g_object_set_data(G_OBJECT(plugin), "launch-plugin", launch_plugin);
